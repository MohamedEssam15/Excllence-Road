<?php

return [
    "appName" => "Excellence Road",
    "Menu" => "Menu",
    "Dashboard" => "Dashboard",
    "search" => "Search ...",

    //courses
    "Courses" => "Courses",
    "allCourses" => "All Courses",
    "activeCourses" => "Active Courses",
    "pendingCourses" => "Pending Courses",
    "expiredCourses" => "Expired Courses",
    "addCourses" => "Add Course",
    "allCoursesTeacher" => "All courses thats belong to you",
    "allCoursesAdmin" => "All courses in the system",
    "id" => 'ID',
    "name" => 'Name',
    "image" => 'Image',
    "description" => 'Description',
    "price" => 'Price',
    "teacherCommistion" => 'Teacher Commistion',
    "category" => 'Category',
    "level" => 'Level',
    "specificTo" => 'Specific To',
    "teacherName" => 'Teacher Name',
    "status" => 'Status',
    "actions" => 'Actions',
    "notSpecific" => 'Not Specific',
    "acceptionFormFor" => 'Acception Form For',
    "courseAccepted" => 'The Course has been accepted',
    "courseModified" => 'The Course has been modified',
    "courseCancelled" => 'The Course has been cancelled',
    "addToPopularCourses" => 'Add to popular courses',
    "cancell" => "Cancell",
    "accept" => "Accept",
    "show" => "Show",
    "edit" => "Edit",
    "close" => "Close",
    "areYouSure" => "Are you sure about cancel this course",
    "cancelledCourses" => "Cancelled courses",
    "backToPending" => "The course will return back to pending courses",
    "returnedToPending" => "The course returned back to pending courses",
    "returnBack" => "ٌReturn back",
    "startDate" => "Start Date",
    "endDate" => "End Date",
    'isPopular' => "Is Popular",
    'yes' => "Yes",
    'no' => "No",
    'currency' => "SAR",
    'arName' => 'Arabic Name',
    'enName' => 'English Name',
    'arDescription' => 'Arabic Description',
    'enDescription' => 'English Description',
    'reset' => 'Reset',
    'submit' => 'Submit',
    'isMobileOnly' => 'View in mobile only',
    'addedToPopularCourses' => 'Added to popular courses',
    'isSpecificForMobile' => 'For Mobile only',
    "showCourse" => 'Show Course',
    "units" => 'Units',
    'lessons' => 'Lessons',
    'lessonType' => 'Lesson Type',
    'meetingDate' => 'meeting Date',
    'meetingLink' => ' Meeting Link',
    'videoLink' => 'video Link',
    'video' => 'Video',
    'meeting' => 'Meeting',
    'trailer' => 'Trailer',
    'reviews' => 'Reviews',
    'reviewsCount' => 'Review',
    'bestSellerCourses' => 'Most sold courses',
    'bestSellerCoursesInfo' => 'Most Sold Courses in the current time',
    'bestSellerPackages' => 'Most sold packages',
    'bestSellerPackagesInfo' => 'Most Sold Packages in the current time',
    'purchasedCount' => 'Purchased Count',
    'totalRevenue' => 'Total Revenue',
    'fromLastMonth' => 'From Last Month',
    'topStudents' => 'Top Students',
    'topTeachers' => 'Top Teachers',
    'ordersCompleted' => 'Orders Completed',
    'sortBy' => 'Sort By',
    'yearly' => 'Yearly',
    'monthly' => 'Monthly',
    'daily' => 'Daily',
    'compareBetweenPackagesAndCourses' => 'Compare Between Packages And Courses',
    'coursecountable' => 'Courses',
    'topSellingCourses' => 'Top Selling Courses',
    'topSeller' => 'Top Seller',
    'customerName' => 'Customer Name',
    'latestTransactions' => 'Latest Transactions',
    //packages
    "packages" => "Packages",
    "activePackages" => "Active Packages",
    "addPackages" => "Add Package",
    "expiredPackages" => "Expired Packages",
    "addToPopularPackages" => "Add To Popular Packages",
    "packgeImage" => "Package Cover",
    "currentImage" => "Current Cover",
    "editPackages" => "Edit Package",
    "showPackage" => "Show Packages",
    'categories' => 'Categories or Topics',
    'addCategory' => 'Add Category',
    'allCategories' => 'All Categories',
    'editCategory' => 'Edit Category',
    'categoryName' => 'Category Name',
    'noCategories' => 'No Categories',
    'arCategoryName' => 'Arabic Category Name',
    'enCategoryName' => 'English Category Name',
    'noRevenue' => 'No Revenue',
    'discountPercentage' => 'Discount Percentage',
    'noCourses' => 'No Courses',
    'noPackages' => 'No Packages',
    'contentName' => 'Content Name',
    'featureContent' => 'Feature Content',
    'type' => 'Type',
    'subject' => 'Subject',
    'delete' => 'Delete',
    'add' => 'Add',
    'noFeatureContent' => 'No Feature Content',
    'contentType' => 'Content Type',
    'photo' => 'Photo',
    'video' => 'Video',
    'addFeatureContent' => 'Add Feature Content',
    'file' => 'File',
    'assignToExistingContent' => 'Assign To Existing Content',
    'content' => 'Content',
    'selectItem' => 'Select Item',
    'deletePackage' => 'Delete Package',

    //users
    "users" => "Users",
    "teachers" => "Teachers",
    "activeTeachers" => "Active Teachers",
    "pendingTeachers" => "Pending Teachers",
    "students" => "Students",
    "activeStudents" => "Active Students",
    "blockedStudents" => "Blocked Students",
    "blockedTeachers" => "Blocked Teachers",
    "admins" => "Admins",
    "allAdmins" => "All Admins",
    "addAdmin" => "Add Admin",
    "role" => "Role",
    "coursesNumber" => "Courses Number",
    "block" => "Block",
    "profile" => "Profile",
    "noUsers" => "No Users",
    "phone" => "Phone",
    "courseCount" => "Course Count",
    "notFound" => "Not Found",
    "certificates" => "Certificates",
    "certificateName" => "Certificate Name",
    "noCertificates" => "No Certificates",
    "download" => "Download",
    "active" => "Active",
    "blocked" => "Blocked",
    "unblock" => "UnBlock",
    "areYouSureBlock" => "Are you sure about block this user",
    "areYouSureAccept" => "Are you sure about Accept this user",
    "areYouSureDelete" => "Are you sure about delete this content",
    "transaction" => 'Transactions',
    "payments" => "Payments",
    "orders" => "Orders",
    "bestSeller" => "Most Seller",
    "monthlyCommission" => "Monthly revenue",
    "currentCommission" => "Current Revenue",
    "contactUsMessages" => "Contact Us Messages",
    "noContactUsMessages" => "No Contact Us Messages",
    "name" => "Name",
    "email" => "Email",
    "phone" => "Phone",
    "message" => "Message",
    "sentAt" => "Sent At",
    "discountAdded" => "Discount Added",
    "discountRemoved" => "Discount Removed",
    "addDiscount" => "Add Discount",
    "removeDiscount" => "Remove Discount",
    "areYouSureRemoveDiscount" => "Are you sure about remove this discount",
    "discountType" => "Discount Type",
    "percentage" => "Percentage",
    "fixedPrice" => "Fixed",
    "discount" => "Discount",
    "newPrice" => "New Price",
    "inProgressPackages" => "In Progress Packages",
    "inProgressCourses" => "In Progress Courses",
    "selectType" => "Select Type",
    "selectItem" => "Select Item",
    "select" => "Select",
    "addFreeCourseOrPackage" => "Add Free Course Or Package",
    "pleaseSelectBothFields" => "Please select both fields",
    "loading" => "Loading",
    "select" => "Select",
    "addFreeCourseOrPackageTo" => "Add Free Course Or Package To",
    'addedBy' => 'Discounted By',
    "addAdmin" => "Add Admin",
    'profileImage' => 'Profile Image',
    'password' => 'Password',
    'permissions' => 'Permissions',
    'noPermissions' => 'No Permissions',
    'editCourse' => 'Edit Course',
    'AcceptReject' => 'Accept/Reject Course',
    'returnBackCourse' => 'Return Back Course',
    'courseInfo' => 'Course Info',
    'reactivePackage' => 'Reactive Package',
    'deleteFeatureContent' => 'Delete Feature Content',
    'acceptRejectTeacher' => 'Accept / Reject Teacher',
    'blockUnblockTeacher' => 'Block / Unblock Teacher',
    'blockUnblockStudent' => 'Block / Unblock Student',
    'showTeacher' => 'Show Teacher',
    'teachersTransactions' => 'Teachers Transactions',
    'editAdmin' => 'Edit Admin',

    "orderNumber" => "Order Number",
    "amount" => "Amount",
    "client" => "Client Name",
    "paidFor" => "Bought",
    "paidForName" => "Name",
    "noOrders" => "No Orders",
    "paidDate" => "Order Date",
    "completed" => "Completed",
    "failed" => "Failed",
    "package" => "package",
    "course" => "course",
    "currentMonthRevenue" => "Current Month Revenue",
    "totalRevenueTillNow" => "Total Revenue Till Now",
    "soldCoursesCount" => "Courses Count",

    "monthlyRevenue" => "Monthly Revenue",
    "latestRevenue" => "Latest Revenue",
    "monthRevenue" => "Month Revenue",
    "date" => "Date",
    "orderDate" => "Order Date",
    "revune" => "Revenue",
    "from" => "From",
    "allRevenue" => "All Revenue",
    "noRevunue" => "No Revunue",
    "revenue" => "Revunue",

    "verfyEmail" => "Verify Your Email Address",
    "Email" => "Email",
    "Profile" => "Profile",
    "Notifications" => "Notifications",
    "Search" => "Search",
    "Login" => "Login",
    "Register" => "Register",
    "Recover_Password" => "Recover Password",
    'noNotifications' => 'No Notifications',
    'newMessageFrom' => 'New Message From',
    'haveContactUsMessage' => 'Have Contact Us Message',
    'newCourseAdded' => 'New Course Added',
    'thereIsNewCourseAdded' => 'There Is New Course Added',
    'newTeacherAdded' => 'New Teacher Added',
    'thereIsNewTeacher' => 'There Is New Teacher',
    'wantToJoin' => 'Want To Join',





    "Layouts" => "Layouts",
    "Horizontal" => "Horizontal",
    "Dark_Sidebar" => "Dark Sidebar",
    "Compact_Sidebar" => "Compact Sidebar",
    "Icon_Sidebar" => "Icon Sidebar",
    "Boxed_Width" => "Boxed Width",
    "Preloader" => "Preloader",
    "Colored_Sidebar" => "Colored Sidebar",
    "Apps" => "Apps",
    "Calendar" => "Calendar",
    "Chat" => "Chat",
    "Ecommerce" => "Ecommerce",
    "Products" => "Products",
    "Product_Detail" => "Product Detail",
    "Orders" => "Orders",
    "Customers" => "Customers",
    "Cart" => "Cart",
    "Checkout" => "Checkout",
    "Shops" => "Shops",
    "Add_Product" => "Add Product",
    "Inbox" => "Inbox",
    "Read_Email" => "Read Email",
    "Invoices" => "Invoices",
    "Invoice_List" => "Invoice List",
    "Invoice_Detail" => "Invoice Detail",
    "Contacts" => "Contacts",
    "User_Grid" => "User Grid",
    "User_List" => "User List",
    "Pages" => "Pages",
    "Authentication" => "Authentication",
    "Lock_Screen" => "Lock Screen",
    "Utility" => "Utility",
    "Starter_Page" => "Starter Page",
    "Maintenance" => "Maintenance",
    "Coming_Soon" => "Coming Soon",
    "Timeline" => "Timeline",
    "FAQs" => "FAQs",
    "Pricing" => "Pricing",
    "Error_404" => "Error 404",
    "Error_500" => "Error 500",
    "Components" => "Components",
    "UI_Elements" => "UI Elements",
    "Alerts" => "Alerts",
    "Buttons" => "Buttons",
    "Cards" => "Cards",
    "Carousel" => "Carousel",
    "Dropdowns" => "Dropdowns",
    "Grid" => "Grid",
    "Images" => "Images",
    "Lightbox" => "Lightbox",
    "Modals" => "Modals",
    "Range_Slider" => "Range Slider",
    "Session_Timeout" => "Session Timeout",
    "Progress_Bars" => "Progress Bars",
    "Sweet_Alert" => "Sweet-Alert",
    "Tabs_Accordions" => "Tabs & Accordions",
    "Typography" => "Typography",
    "Video" => "Video",
    "General" => "General",
    "Colors" => "Colors",
    "Rating" => "Rating",
    "Forms" => "Forms",
    "Basic_Elements" => "Basic Elements",
    "Validation" => "Validation",
    "Advanced_Plugins" => "Advanced Plugins",
    "Editors" => "Editors",
    "File_Upload" => "File Upload",
    "Xeditable" => "Xeditable",
    "Repeater" => "Repeater",
    "Wizard" => "Wizard",
    "Mask" => "Mask",
    "Tables" => "Tables",
    "Bootstrap_Basic" => "Bootstrap Basic",
    "Datatables" => "Datatables",
    "Responsive" => "Responsive",
    "Editable" => "Editable",
    "Charts" => "Charts",
    "Apex" => "Apex",
    "Chartjs" => "Chartjs",
    "Flot" => "Flot",
    "Jquery_Knob" => "Jquery Knob",
    "Sparkline" => "Sparkline",
    "Icons" => "Icons",
    "Unicons" => "Unicons",
    "Boxicons" => "Boxicons",
    "Material_Design" => "Material Design",
    "Dripicons" => "Dripicons",
    "Font_Awesome" => "Font Awesome",
    "Maps" => "Maps",
    "Google" => "Google",
    "Vector" => "Vector",
    "Leaflet" => "Leaflet",
    "Multi_Level" => "Multi Level",
    "Level_1.1" => "Level 1.1",
    "Level_1.2" => "Level 1.2",
    "Level_2.1" => "Level 2.1",
    "Level_2.2" => "Level 2.2",
    "Mark_read" => "Mark all as read",
    "order_placed" => "Your order is placed",
    "languages_grammar" => "If several languages coalesce the grammar",
    "3_min_ago" => "3 min ago",
    "simplified_English" => "It will seem like simplified English.",
    "1_hours_ago" => "1 hours ago",
    "item_shipped" => "Your item is shipped",
    "friend_occidental" => "As a skeptical Cambridge friend of mine occidental.",
    "View_More" => "View More",
    "View_Profile" => "View Profile",
    "My_Wallet" => "My Wallet",
    "Settings" => "Settings",
    "Lock_screen" => "Lock screen",
    "Sign_out" => "Sign out",
    "Extra_pages" => "Extra pages",
    "Apex_Charts" => "Apex Charts",
    "Flot_Charts" => "Flot Charts",
    "Sparkline_Chart" => "Sparkline Chart",
    "Form_Advanced_Plugins" => "Form Advanced Plugins",
    "Form_editor" => "Form editor",
    "Form_Mask" => "Form Mask",
    "Form_Repeater" => "Form Repeater",
    "Form_File_Upload" => "Form File Upload",
    "Form_wizard" => "Form wizard",
    "Horizontal_Layout" => "Horizontal Layout",
    "Google_maps" => "Google maps",
    "Vector_Maps" => "Vector Maps",
    "Maintenance_Page" => "Maintenance Page",
    "Editable_Table" => "Editable Table",
    "SweetAlert_2" => "SweetAlert 2",
    "Vertical" => "Vertical",
    "Boxed_Horizontal" => "Boxed Horizontal",
    "Dark_Topbar" => "Dark Topbar",
    "New" => "New",
    "File_Manager" => "File Manager",
    "Offcanvas" => "Offcanvas",
    "Placeholders" => "Placeholders",
    "Utilities" => "Utilities",
    "Toasts" => "Toasts"
];
