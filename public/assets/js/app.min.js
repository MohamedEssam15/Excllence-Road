    ...
    function layoutSetting(id) {
        ...
        $('#mode-setting-btn').on('click', function (e) {
            if (body.hasAttribute("data-bs-theme") && body.getAttribute("data-bs-theme") == "dark") {
                ...
                checkTheAtrributeIsExistAndUpdateRadio('layout-mode-dark')
        ...
        if (currentLang === 'ar') {
            switchLayoutDirection('rtl');
        } else {
            switchLayoutDirection('ltr');
        }
        // Event listener for manual layout direction change (if needed)
        $("input[name='layout-direction']").on('change', function () {
            switchLayoutDirection($(this).val());
        });
    }
